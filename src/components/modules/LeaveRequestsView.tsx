    ...
    <div className="space-y-6 pb-12 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-6 rounded-2xl bg-white dark:bg-stone-900 border border-stone-200/80 dark:border-stone-800 shadow-xs">
        <div>
          <h2 className="text-lg font-bold text-stone-900 dark:text-white flex items-center gap-2">
            <MailCheck className="w-5 h-5 text-[#4a0404] dark:text-rose-400" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredRequests.length === 0 ? (
          <div className="col-span-full p-12 text-center rounded-2xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 text-stone-400">
            <MailCheck className="w-10 h-10 mx-auto text-stone-300 dark:text-stone-600 mb-2" />
          ...
          filteredRequests.map((req) => (
            <div
              key={req.id}
              className="rounded-2xl bg-white dark:bg-stone-900 border border-stone-200/80 dark:border-stone-800 shadow-xs p-4 flex flex-col justify-between space-y-3.5"
            >
              <div className="space-y-3">
                {/* Header item */}
                <div className="flex items-start justify-between">
                  <div>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-stone-700 dark:text-stone-300 bg-stone-100 dark:bg-stone-800 px-2 py-0.5 rounded-md border border-stone-200 dark:border-stone-700">
                      Surat {req.type}
                    </span>
                    <h3 className="font-bold text-sm text-stone-900 dark:text-white mt-1.5">
                      {req.studentName}
                    </h3>
                    <p className="text-xs text-stone-500">{req.className}</p>
                  </div>
                ...
                {/* Dates & Wali */}
                <div className="p-3 rounded-xl bg-stone-50 dark:bg-stone-800/50 space-y-1 text-xs text-stone-600 dark:text-stone-300 border border-stone-100 dark:border-stone-800">
                  <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-stone-700 dark:text-stone-300 mt-1 leading-relaxed bg-stone-50/50 dark:bg-stone-900/50 p-2.5 rounded-xl border border-stone-100 dark:border-stone-800 italic">
                    "{req.reason}"
                  </p>
                </div>
              </div>
                ...
                {req.status === 'Pending' ? (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex-1 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 py-1.5 bg-stone-100 dark:bg-stone-800 hover:bg-rose-50 dark:hover:bg-rose-950 text-stone-700 dark:text-stone-300 hover:text-rose-600 rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1.5"
                    >
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-xl text-xs font-semibold text-stone-900 dark:text-white"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-stone-50 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-xl text-xs text-stone-900 dark:text-white font-semibold"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-stone-300 mb-1">
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-stone-100 dark:border-stone-800">
              ...
              <button
                type="submit"
                className="px-5 py-2 bg-[#4a0404] hover:bg-[#380303] text-white rounded-xl text-xs font-bold"
              >
                Kirim Pengajuan
              </button>
            </div>
